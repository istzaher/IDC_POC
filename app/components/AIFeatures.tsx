...
import { Bot, Search, Shield, Link, Lightbulb, Zap, CheckCircle, AlertTriangle, TrendingUp, Database, XCircle } from 'lucide-react'
...
function DuplicateDetectionDemo() {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      const matches = await aiService.detectDuplicates({ description: input })
      setResults(matches)
      setLoading(false)
    }, 500)
  ...
  return (
    <div className="mt-4 p-4 bg-white border rounded">
      ...
      {touched && !loading && input && results.length === 0 && (
        <div className="bg-green-50 border border-green-200 rounded p-3">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle className="w-4 h-4 text-green-600" />
            <span className="text-sm font-medium text-green-800">No Duplicates Found</span>
          </div>
          <p className="text-xs text-green-700">This entry appears unique.</p>
        </div>
      )}
      {results.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-orange-800">Potential Duplicate{results.length > 1 ? 's' : ''} Found</span>
          </div>
          {results.map((match, idx) => (
            <p key={idx} className="text-xs text-orange-700">
              {Math.round(match.similarity * 100)}% similarity with "{match.material.description}" ({match.material.materialCode})
            </p>
          ))}
        </div>
      )}
...
function FieldSuggestionDemo() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const handler = setTimeout(async () => {
      // Simulate AI suggestion: use aiService.suggestMaterialType if available, else fallback
      let sugg: string[] = []
      if (aiService.suggestMaterialType) {
        sugg = await aiService.suggestMaterialType(input)
      } else {
        // fallback: simple keyword match
        if (input.toLowerCase().includes('pipe')) sugg = ['PIPE', 'TUBE']
        else if (input.toLowerCase().includes('rod')) sugg = ['ROD', 'BAR']
        else if (input.toLowerCase().includes('cable')) sugg = ['CABLE', 'WIRE']
        else sugg = ['MATERIAL']
      }
      setSuggestions(sugg)
      setLoading(false)
  ...
  return (
    <div className="mt-4 p-4 bg-white border rounded">
      ...
      />
      {loading && <div className="text-xs text-gray-500">Analyzing...</div>}
      {touched && !loading && input && suggestions.length === 0 && (
        <div className="bg-red-50 border border-red-200 rounded p-3">
          <div className="flex items-center gap-2 mb-2">
            <XCircle className="w-4 h-4 text-red-600" />
            <span className="text-sm font-medium text-red-800">No Suggestions</span>
          </div>
          <p className="text-xs text-red-700">No suggestions found for this input.</p>
        </div>
      )}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded p-3">
          ...
          <div className="space-y-1">
            {suggestions.map((s, idx) => (
              <button key={idx} className="block text-xs text-blue-700 hover:text-blue-900">{s}</button>
            ))}
          </div>
        </div>
      )}
